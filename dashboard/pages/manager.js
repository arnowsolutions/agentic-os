...
async function cmdApprove() {
  ...
  try {
    ...
    if (atCap.length > 0) {
      hasItems = true;
      html += `<div class="r-line" style="margin-top:8px"><span class="r-tag r-tag-red">!</span> <strong>GME Cap Reached (${atCap.length})</strong></div>`;
    ...
    if (nearCap.length > 0) {
      hasItems = true;
      html += `<div class="r-line" style="margin-top:8px"><span class="r-tag r-tag-yellow">!</span> <strong>Near GME Cap (${nearCap.length})</strong></div>`;
...
async function cmdGap() {
  let html = `<div class="r-header">! Coverage Gaps</div>`;
  ...
  try {
    ...
    if (now.oncall && now.oncall.length > 0) {
      ...
      for (const [hosp, entries] of Object.entries(byHospital)) {
        ...
        html += entries.map(e => `Primary: ${escapeHtml(e.primary_attending || '—')}${e.backup_attending && e.backup_attending !== 'None' ? ` (Backup: ${escapeHtml(e.backup_attending)})` : ' ! No backup'}`).join(' · ');
        html += `</div>`;
      }
    } else {
    ...
    if (nearCap.length > 0) {
      html += `<div class="r-line" style="font-size:12px;color:var(--red)">! ${nearCap.length} resident(s) near/exceeded GME cap:</div>`;
      for (const r of nearCap) {
