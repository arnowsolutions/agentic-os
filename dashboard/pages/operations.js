/* ═══════════════════════════════════════════════════════════════════════
   Operations — System metrics, agents & recent activity
   (Migrated from the pre-launchpad dashboard, 2026-09-12 redesign PR3.)
...
async function renderOperations(target) {
  ...
  content.innerHTML = `
    <div class="page-header">
      <div class="page-header-left">
        ...
        <p class="page-subtitle">System metrics, agents and recent activity</p>
      </div>
      <div class="btn-group">
        <button class="btn btn-primary btn-sm" onclick="opRunQuickSkill()">Run Skill</button>
        <button class="btn btn-sm" onclick="navigate('backups')">Backup</button>
        <button class="btn btn-ghost btn-sm" onclick="renderOperations()">Refresh</button>
      </div>
    </div>
    <div id="opStats" class="grid grid-4 mb-4"></div>
    <div class="grid grid-3 mb-4">
      <div class="card" style="grid-column:span 2">
        <div class="card-header"><span class="card-title">Agent Status</span></div>
        <div id="opAgentList"><div class="skeleton" style="height:80px"></div></div>
      </div>
      <div class="card">
        <div class="card-header"><span class="card-title">Recent Activity</span></div>
        <div id="opRecentActivity"><div class="skeleton" style="height:80px"></div></div>
      </div>
    </div>
    <div id="opQuickActions" class="mb-4"></div>
  `;
  ...
  try {
    ...
    // ── Agent status ──
    document.getElementById('opAgentList').innerHTML = `
    ...
    // ── Recent activity ──
    document.getElementById('opRecentActivity').innerHTML = entries.length === 0
    ...
    // ── Quick actions ──
    document.getElementById('opQuickActions').innerHTML = `
      <div class="card">
    ...
    document.getElementById('opStats').innerHTML = `<div class="card" style="grid-column:1/-1"><div class="empty-state"><div class="empty-state-icon">—</div><div class="empty-state-title">Connection Error</div><div class="empty-state-desc">${escapeHtml(err.message)}</div><button class="btn btn-primary mt-3" onclick="renderOperations()">Retry</button></div></div>`;
  }
}

// ── Quick Run Skill ──
async function opRunQuickSkill() {
  showModal('Run Skill', `
    <div class="form-group">
      <label class="form-label">Skill</label>
      <select id="opQrSkill" class="form-input">
        <option value="">Select a skill...</option>
      </select>
    </div>
    <div class="form-group">
      <label class="form-label">Input (optional)</label>
      <textarea id="opQrInput" class="form-textarea" rows="3" placeholder="Enter input for the skill..."></textarea>
    </div>
  `, `
    <button class="btn btn-ghost" onclick="closeModal()">Cancel</button>
    <button class="btn btn-primary" onclick="opExecuteQuickRun()">Run</button>
  `);
  try {
    const skills = await api.getSkills();
    const select = document.getElementById('opQrSkill');
    skills.forEach(s => {
...
async function opExecuteQuickRun() {
  const name = document.getElementById('opQrSkill').value;
  const input = document.getElementById('opQrInput').value;
