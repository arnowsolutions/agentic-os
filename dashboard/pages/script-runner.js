async function renderScriptRunner(target) {
  ...
  content.innerHTML = `
    ...
    <div class="sr-layout">
      <div class="sr-panel sr-controls-panel">
        ...
        <div class="sr-custom">
          ...
          <button class="btn btn-primary" onclick="runCustomScript()" id="srRunBtn">▸ Run</button>
        </div>
      </div>
      <div class="sr-panel sr-output-panel">
        <div class="sr-output-header">
          ...
          <div>
            <button class="btn btn-ghost btn-xs" onclick="clearSrOutput()">✕ Clear</button>
    ...
    <style>
      ...
      .sr-script-btn:hover { background: var(--hover-bg-strong); border-color: var(--primary); }
  ...
  const SCRIPTS = [
    ...
    { name: 'Sync Schedule to Calendar', icon: '▸', path: 'python3 /workspace/agentic-os/daily_sync.py' },
...
async function runScriptWithOutput(cmd, name) {
  ...
  title.textContent = `▸ Running: ${name || cmd}`;
