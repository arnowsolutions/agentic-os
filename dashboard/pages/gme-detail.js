async function renderGmeDetail(target) {
  ...
  content.innerHTML = `
    ...
    <style>
      ...
      .gd-table td { padding:8px 10px; border-bottom:1px solid var(--border-soft); }
      .gd-table tr:hover td { background:var(--hover-bg); }
      .gd-bar { height:6px; border-radius:3px; background:var(--fill-muted); overflow:hidden; width:100px; display:inline-block; vertical-align:middle; }
  ...
  try {
    ...
    const ANNUAL_BUDGET = res.annual_limit || 1250;
    if (!residents.length) {
      document.getElementById('gdSummary').innerHTML =
        '<div class="empty-state"><div class="empty-state-icon">!</div><div class="empty-state-title">No GME data available</div><div class="empty-state-desc">The live source returned no residents. Use Sync now, then check the Unified Platform reimbursement data.</div></div>';
      return;
    }
    ...
    document.getElementById('gdSummary').innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:20px;color:var(--text-muted)">! ${escapeHtml(e.message)}</div>`;
    document.getElementById('gdTable').innerHTML = '';
  }
}
