async function renderMorningBriefing() {
  ...
  content.innerHTML = `
    ...
    <style>
      ...
      .bc-row { display:flex; justify-content:space-between; padding:4px 0; font-size:12px; border-bottom:1px solid var(--border-soft); }
      ...
      .bc-tag.email { background:rgba(20,184,166,0.15); color:#14b8a6; }
      .bc-tag.local { background:rgba(99,110,114,0.15); color:#636e72; }
      .bc-tag.all { background:rgba(20,184,166,0.25); color:#0d9488; }
      ...
      .bc-cron-table td { padding:5px 8px; border-bottom:1px solid var(--border-soft); vertical-align:top; }
      .bc-cron-table tr:hover { background:var(--hover-bg); }
  ...
  try {
    ...
    document.getElementById('bcGrid').innerHTML = `
      <div class="bc-card" style="grid-column:1/-1">
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap">
          <div style="flex:1;min-width:110px;padding:10px;background:var(--hover-bg);border-radius:var(--radius-sm);text-align:center">
    ...
    document.getElementById('bcGrid').innerHTML = `<div class="bc-card" style="grid-column:1/-1;text-align:center;padding:32px;color:var(--red)">! ${escapeHtml(e.message)}</div>`;
  }
}
