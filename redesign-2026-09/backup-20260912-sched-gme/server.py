...
def _get_db_conn():
    ...
    Tries 127.0.0.1 (systemd/host deployment) first, then the Docker
    gateway IP (container deployment, e.g. hermes-webui-gsga-agentic-os-1
    reaches the host's postgres via its bridge gateway).
    """
    import psycopg2
    pw = os.environ.get("POSTGRES_PASSWORD", "")
    if not pw:
...
async def create_subi_exit_interview(request: Request):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute('''
            INSERT INTO subi_exit_interviews (interviewee, recipient_email, interview_date, interview_time, duration_minutes, notes)
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id
        ''', (
...
async def update_subi_exit_interview(row_id: int, request: Request):
    ...
    try:
        ...
        values.append(row_id)
        cur.execute(
            f"UPDATE subi_exit_interviews SET {', '.join(updates)} WHERE id = %s",
            values
        )
        conn.commit()
        affected = cur.rowcount
...
async def delete_subi_exit_interview(row_id: int):
    ...
    try:
        ...
        if not conn:
            return {"error": "No DB password"}
        cur = conn.cursor()
        cur.execute("DELETE FROM subi_exit_interviews WHERE id = %s", (row_id,))
        conn.commit()
        affected = cur.rowcount
        cur.close()
        conn.close()
