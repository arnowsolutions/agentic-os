...
def _resolve_opencode_bin() -> Optional[str]:
    """Return the first existing opencode CLI entrypoint, or None.

    The Agentic OS container ships no node/npm of its own — the toolchain
    lives in the shared workspace volume (.tools/opencode) with a
    self-contained launcher (oc-run) that fixes HOME/XDG + PATH.
    """
    candidates = [
        "/workspace/agentic-os/.tools/opencode/oc-run",
...
def execute_agent(agent: str, message: str) -> str:
    ...
    try:
        if agent == "opencode":
            oc_bin = _resolve_opencode_bin()
            if not oc_bin:
                return _agent_unavailable(
                    "opencode",
                    message,
                    "opencode CLI is not installed for Agentic OS (expected at "
                    "/workspace/agentic-os/.tools/opencode/oc-run).",
                )
            try:
                code, out, err = run_cli(
                    [oc_bin, "run", "--format", "json", message], timeout=90
                )
