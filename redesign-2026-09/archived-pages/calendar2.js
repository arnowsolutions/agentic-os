...
async function renderCalendar() {
  ...
  try {
    ...
    // Load todos from the events response (includes task-list.json)
...
// ═════════════════════════════════════════════════════════════════
//  TODO PANEL — reads /workspace/task-list.json via /api/tasks
// ═════════════════════════════════════════════════════════════════

async function loadTodos() {
  try {
    const resp = await fetch('/api/calendar/todos').then(r => r.json()).catch(() => []);
    let items = [];
    // Handle both formats: flat array from task-list.json, or {columns:{...}} from kanban
    if (Array.isArray(resp)) {
      items = resp;
    } else if (resp.columns) {
      for (const [status, tasks] of Object.entries(resp.columns)) {
        if (Array.isArray(tasks)) tasks.forEach(t => items.push(t));
      }
    }
...
async function toggleTodo(id, complete) {
  try {
    await fetch(`/api/calendar/todos/${encodeURIComponent(id)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
async function deleteTodo(id) {
  try {
    await fetch(`/api/calendar/todos/${encodeURIComponent(id)}`, { method: 'DELETE' });
    await loadTodos();
  } catch (err) {
