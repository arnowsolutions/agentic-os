...
def parse_gr_data():
    """Legacy name — schedule now comes ONLY from the canonical store
    (unified.grand_rounds). Fails loudly if the DB is unreachable."""
    from gr_schedule import fetch_rows
    return fetch_rows(include_tb=True)
...
def get_schedule_from_db():
    """CANONICAL schedule source: unified.grand_rounds (single source of truth).
    urology_qgenda.grand_rounds_schedule retired 2026-09-09 — do not restore."""
    from gr_schedule import fetch_rows
    return fetch_rows(include_tb=True)
...
def build_monday_body(event):
    ...
    def dr_resident(name):
        # Residents are stored by FIRST name in the schedule — resolve to
        # "Dr. LastName" through the canonical map (gr_schedule.dr_resident).
        from gr_schedule import dr_resident as _dr_resident
        v = _dr_resident(name)
        return esc(v) if v else "TBD"
    return (
        ...
        f"<tr><td><strong>Resident</strong></td><td>{dr_resident(event['resident'])}</td></tr>"
...
def _build_event_rows(events, test_mode, test_email):
    ...
    for month_label, month_events in by_month.items():
        rows_html += f'<tr style="background:#f8f8f9"><td colspan="5" style="padding:10px 14px;font-weight:700;font-size:14px;color:#b45309">{month_label}</td></tr>\n'
        for ev in month_events:
            ...
            rows_html += f'''<tr id="row-{event_id}" style="border-bottom:1px solid #e4e4e7">
  ...
  <td style="padding:10px 14px;font-size:12px;color:#71717a;max-width:300px;overflow:hidden;text-overflow:ellipsis">{display_subject}</td>
  ...
  <td style="padding:10px 14px;white-space:nowrap">
    ...
    <button onclick="openEditor('{event_id}')" style="display:inline-block;background:transparent;color:#71717a;border:1px solid #d4d4d8;padding:5px 12px;border-radius:6px;cursor:pointer;font-size:12px;margin-left:6px">Edit</button>
  </td>
</tr>
'''
    return rows_html
...
def generate_html_page(monday_events, gr_events, test_mode=True, test_email=TEST_EMAIL):
    ...
    # Load server-side sent data: CANONICAL DB tracker (unified.grand_rounds
    # *_sent_at) is the single source going forward. Legacy progress/send-log
    # files are read ONLY to backfill badges for sends that predate the tracker.
    ...
    try:
        ...
        for rec in fetch_dicts():
            if rec.get("mon_date") and rec.get("mon_invite_sent_at"):
                sent_dates["monday"].add(rec["mon_date"])
            if rec.get("fri_date") and rec.get("fri_invite_sent_at"):
                sent_dates["grand_rounds"].add(rec["fri_date"])
    except Exception:
        pass
    try:
    ...
    # Build schedule editor data from CANONICAL store (unified.grand_rounds)
    schedule_rows = []
    try:
        from gr_schedule import fetch_dicts
        for r in fetch_dicts():
            schedule_rows.append({
                "db_id": r["id"],
                "mon_date": r["mon_date"] or "",
                "mon_topic": r["mon_topic"] or "",
                "resident": r["mon_resident"] or "",
                "attending": r["mon_attending"] or "",
                "fri_date": r["fri_date"] or "",
                "gr_7_8": r["fri_gr7"] or "",
                "gr_8_9": r["fri_gr8"] or "",
            })
    except Exception:
...
<style>
  * {{ margin:0; padding:0; box-sizing:border-box }}
  body {{ background:#ffffff; color:#18181b; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif; padding:20px; max-width:900px; margin:0 auto }}
  ...
  .toggle-bar .reset-btn {{ margin-left:auto; background:#e4e4e7; color:#71717a; border:1px solid #d4d4d8; padding:5px 12px; border-radius:6px; cursor:pointer; font-size:12px }}
  .toggle-bar .reset-btn:hover {{ background:#d4d4d8; color:#18181b }}
  /* Sent status */
  ...
  .edit-badge {{ display:inline-flex; align-items:center; gap:4px; background:#fef3c7; color:#b45309; padding:3px 8px; border-radius:999px; font-size:10px; font-weight:600 }}
  /* Editor modal */
  ...
  .modal {{ background:#ffffff; border:1px solid #e4e4e7; border-radius:12px; width:100%; max-width:700px; padding:24px; box-shadow:0 20px 60px rgba(0,0,0,0.5) }}
  .modal h2 {{ font-size:18px; margin-bottom:16px; color:#b45309 }}
  .modal label {{ display:block; font-size:12px; color:#71717a; margin-bottom:4px; font-weight:600; text-transform:uppercase; letter-spacing:0.5px }}
  .modal input[type=text] {{ width:100%; background:#ffffff; border:1px solid #e4e4e7; color:#18181b; padding:10px 12px; border-radius:6px; font-size:14px; margin-bottom:16px }}
  .modal .body-editor {{ width:100%; min-height:300px; background:#fff; color:#1a1a1a; border:1px solid #e4e4e7; border-radius:6px; padding:16px; font-size:14px; line-height:1.6; overflow-y:auto; margin-bottom:16px }}
  ...
  .modal .btn-send:hover {{ background:#f59e0b }}
  .modal .btn-cancel {{ background:#e4e4e7; color:#71717a; border:1px solid #d4d4d8; padding:10px 20px; border-radius:6px; cursor:pointer; font-size:14px }}
  .modal .btn-cancel:hover {{ background:#d4d4d8; color:#18181b }}
  /* Tabs */
  .tabs {{ display:flex; gap:0; margin-bottom:0; border-bottom:2px solid #e4e4e7 }}
  .tab {{ padding:12px 24px; cursor:pointer; font-size:14px; font-weight:600; color:#71717a; border:none; background:none; border-bottom:3px solid transparent; transition:all 0.2s }}
  .tab:hover {{ color:#18181b }}
  .tab.active {{ color:#b45309; border-bottom-color:#b45309 }}
  .tab .badge {{ display:inline-block; background:#e4e4e7; color:#71717a; padding:1px 8px; border-radius:999px; font-size:11px; margin-left:6px }}
  .tab.active .badge {{ background:#f59e0b; color:#0f172a }}
  /* Tab content */
...
{friday_table_html}      </tbody>
  ...
  <div id="tab-edit" class="tab-content">
    ...
    <div style="margin:14px 0;display:flex;gap:12px;align-items:center">
      ...
      <span id="saveStatus" style="color:#71717a;font-size:12px"></span>
    </div>
  ...
  <div class="modal-overlay" id="editorModal" onclick="if(event.target===this)closeEditor()">
    <div class="modal">
      ...
      <div class="modal-actions">
        ...
        <button class="btn-send" onclick="saveEditedChanges()" style="background:#d4d4d8;color:#18181b;border:none;padding:10px 20px;border-radius:6px;cursor:pointer;font-size:14px">Save Changes</button>
  ...
  <script>
    ...
    function initScheduleEditor() {{
      ...
      scheduleData.forEach((row, i) => {{
        ...
        html += `<tr id="sched-row-${{i}}" style="border-bottom:1px solid #e4e4e7" data-db-id="${{dbId}}">`;
        html += `<td style="padding:6px"><input type="text" value="${{row.mon_date}}" data-row="${{i}}" data-col="mon_date" data-db-id="${{dbId}}" style="width:90px;background:#ffffff;border:1px solid #e4e4e7;color:#18181b;padding:4px 6px;border-radius:4px;font-size:12px"></td>`;
        html += `<td style="padding:6px"><input type="text" value="${{row.mon_topic}}" data-row="${{i}}" data-col="mon_topic" data-db-id="${{dbId}}" style="width:220px;background:#ffffff;border:1px solid #e4e4e7;color:#18181b;padding:4px 6px;border-radius:4px;font-size:12px"></td>`;
        html += `<td style="padding:6px"><input type="text" value="${{row.resident}}" data-row="${{i}}" data-col="resident" data-db-id="${{dbId}}" style="width:110px;background:#ffffff;border:1px solid #e4e4e7;color:#18181b;padding:4px 6px;border-radius:4px;font-size:12px"></td>`;
        html += `<td style="padding:6px"><input type="text" value="${{row.attending}}" data-row="${{i}}" data-col="attending" data-db-id="${{dbId}}" style="width:130px;background:#ffffff;border:1px solid #e4e4e7;color:#18181b;padding:4px 6px;border-radius:4px;font-size:12px"></td>`;
        html += `<td style="padding:6px"><input type="text" value="${{row.fri_date}}" data-row="${{i}}" data-col="fri_date" data-db-id="${{dbId}}" style="width:90px;background:#ffffff;border:1px solid #e4e4e7;color:#18181b;padding:4px 6px;border-radius:4px;font-size:12px"></td>`;
        html += `<td style="padding:6px"><input type="text" value="${{row.gr_7_8}}" data-row="${{i}}" data-col="gr_7_8" data-db-id="${{dbId}}" style="width:200px;background:#ffffff;border:1px solid #e4e4e7;color:#18181b;padding:4px 6px;border-radius:4px;font-size:12px"></td>`;
        html += `<td style="padding:6px"><input type="text" value="${{row.gr_8_9}}" data-row="${{i}}" data-col="gr_8_9" data-db-id="${{dbId}}" style="width:200px;background:#ffffff;border:1px solid #e4e4e7;color:#18181b;padding:4px 6px;border-radius:4px;font-size:12px"></td>`;
        html += '</tr>';
      }});
    ...
    async function saveAllChanges() {{
      ...
      if (errors === 0) {{
        status.innerHTML = `<span style="color:#047857">✓ Saved ${{saved}} row(s) to database.</span>`;
      }} else {{
    ...
    function saveEditedChanges() {{
      ...
      status.innerHTML = '<span style="color:#047857">✓ Saved</span>';
    ...
    function discardEdits() {{
      ...
      const status = document.getElementById('editSaveStatus');
      status.innerHTML = '<span style="color:#71717a">Discarded saved edits</span>';
      setTimeout(() => status.textContent = '', 3000);
      const badge = document.getElementById('saved-badge-' + currentEditId);
    ...
    function markSent(eventId) {{
      ...
      persistSentToDb(eventId, false);
      updateRowSent(eventId);
    }}

    // Persist send state to the CANONICAL tracker (unified.grand_rounds)
    function persistSentToDb(eventId, clear) {{
      const parts = eventId.split('_');
      if (parts.length < 2) return;
      const kind = parts[0] === 'mon' ? 'mon_invite' : 'fri_invite';
      fetch('/api/send-tracker/mark', {{
        method: 'POST',
        headers: {{'Content-Type': 'application/json'}},
        body: JSON.stringify({{date: parts[1], kind: kind, clear: !!clear}}),
      }}).catch(() => {{}});
    }}
