...
def _handle_auth(name: str, pin: str, ez_id: str = "", client_ip: str = "", request: Request | None = None, caller_phone: str = ""):
    result = _handle_auth_inner(name, pin, ez_id, client_ip, request, caller_phone)
    status = "success" if result.get("verified") else result.get("next_step", "unknown")
    collector.observe_auth_attempt(status)
    return result


def _handle_auth_inner(name: str, pin: str, ez_id: str = "", client_ip: str = "", request: Request | None = None, caller_phone: str = ""):
    ...
    # PRIORITY -1: VIP phone recognition — skip PIN entirely
    if caller_phone and not ez_id and not pin:
        vip_user = _lookup_by_phone(caller_phone)
        if vip_user:
            role = vip_user.get("role", "")
            display = vip_user.get("display_name", vip_user.get("name", ""))
            _auth_limiter.record(display, True, ip)
...
async def _dispatch_vapi_webhook(request: Request, body: dict):
    ...
    try:
        if fn_name in ("authUser", "verifyCaller"):
            ...
            # Extract caller phone number for VIP recognition
            call_obj = body.get("call", body.get("message", {}).get("call", {})) if isinstance(body, dict) else {}
            caller_phone = ""
            if isinstance(call_obj, dict):
                cust = call_obj.get("customer", {})
                if isinstance(cust, dict):
                    caller_phone = cust.get("number", cust.get("phone", ""))
            ...
            # Try VIP phone recognition first
            if caller_phone and not ez_id and not name:
                vip_user = _lookup_by_phone(caller_phone)
                if vip_user:
                    role = vip_user.get("role", "")
            ...
            r = _handle_auth(name, pin, ez_id=ez_id, caller_phone=caller_phone, request=request)
            ...
            sick_result = _handle_sick_call(fn_args)
            if sick_result.get("status") == "submitted":
                coverage = _auto_find_coverage_for_sick_call(fn_args, sick_result)
                sick_result["coverage"] = coverage
            return {"result": json.dumps(sick_result), "_metric_fn": fn_name}, "ok"
            ...
            if date_from and date_to:
                # Date range: normalize both, iterate days
                from datetime import date, timedelta
                norm_start = vapi_unified._normalize_date(date_from) or date_from
                norm_end = vapi_unified._normalize_date(date_to) or date_to
                try:
                    start = date.fromisoformat(norm_start) if isinstance(norm_start, str) else norm_start
                    end = date.fromisoformat(norm_end) if isinstance(norm_end, str) else norm_end
                except ValueError:
                ...
                # Single date email — normalize and fetch coverage for that date
                from datetime import date
                norm = vapi_unified._normalize_date(date_from) or date_from
                result = vapi_unified.schedule_by_date(norm)
                campuses = result.get("campuses", {})
                try:
                    _pdate = date.fromisoformat(norm)
                    day_name = _pdate.strftime("%A")
                except Exception:
                    day_name = ""
                for cname, cov in campuses.items():
                    if campus_filter and campus_filter.lower() != cname.lower():
                        continue
                    schedule_data.append({
                        "date": result.get("date", norm), "day": day_name,
                        "primary": cov.get("primary", "—"),
                        "backup": cov.get("backup", "—"),
                        "peds": cov.get("peds", "—"),
                        "campus": cname,
            ...
            r = _handle_get_my_dashboard(fn_args)
            return {"result": json.dumps(r), "_metric_fn": fn_name}, "ok"
        elif fn_name == "emailMyDashboard":
            r = _handle_email_my_dashboard(fn_args)
            return {"result": json.dumps(r), "_metric_fn": fn_name}, "ok"
        elif fn_name == "getDirections":
            from modules import vapi_realtime
            origin = fn_args.get("origin", "")
            dest = fn_args.get("destination", "")
            result = vapi_realtime.handle_directions(origin, dest)
            return {"result": json.dumps(result), "_metric_fn": fn_name}, "ok"
        elif fn_name == "emailDirections":
            from modules.vapi_realtime_bridge_patch import _send_directions_email
            email = fn_args.get("email", "")
            origin = fn_args.get("origin", "")
            dest = fn_args.get("destination", "")
            r = _send_directions_email(email, origin, dest)
            return {"result": json.dumps(r), "_metric_fn": fn_name}, "ok"
        elif fn_name == "emailWeather":
            from modules.vapi_realtime_bridge_patch import _send_weather_email
            email = fn_args.get("email", "")
            loc = fn_args.get("location", "Bronx, NY")
            r = _send_weather_email(email, loc)
            return {"result": json.dumps(r), "_metric_fn": fn_name}, "ok"
        elif fn_name == "emailNews":
            from modules.vapi_realtime_bridge_patch import _send_news_email
            email = fn_args.get("email", "")
            topic = fn_args.get("topic", "")
            r = _send_news_email(email, topic)
            return {"result": json.dumps(r), "_metric_fn": fn_name}, "ok"
        else:
...
def _handle_get_my_dashboard(args: dict) -> dict:
    ...
    try:
        ...
        # schedule_person returns {"results": [...], "total": N}
        results = person_sched.get("results", []) if isinstance(person_sched, dict) else (person_sched if isinstance(person_sched, list) else [])
        today_str = date.today().strftime("%Y-%m-%d")
        upcoming_calls = []
        for item in results:
            if isinstance(item, dict) and item.get("date", "9999") >= today_str:
...
def _handle_email_my_dashboard(args: dict) -> dict:
    ...
    try:
        ...
        # Save PDF to temp file (send_email_smart takes file paths, not bytes)
        pdf_path = f"/tmp/dashboard_{dashboard['date']}_{name.replace(' ', '_')}.pdf"
